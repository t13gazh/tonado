...
## Update-Mechanismus (Folgearbeit)

Die 0.3.0-beta-Live-Test-Findings sind in der Welle „Update-Pfad hardenen" (2026-04-23) abgearbeitet: Apply-Bug mit pip-Timeout behoben, Progress-UI inline, `Cache-Control: no-store` auf `/api/system/update/*` + `/info` + `/health`, Rate-Limit-Bucket für Update-Check, sudoers-Minimalregel statt Pauschal-NOPASSWD, Uvicorn auf Localhost. Details im CHANGELOG unter `Unreleased`. Offene Punkte für künftige Wellen:

- [ ] **Frontend-Asset-Hash-Konsistenz bei Rebuilds.** Rebuild-Commits pro Source-Welle decken das im normalen Workflow ab; fragil bleibt der Fall „manueller `git reset --hard` + `git clean -fdx`". Entweder `prestart`-Hook im Service, der `index.html`-Assets gegen `_app/immutable/` prüft und bei Diskrepanz einen `npm run build` triggert — oder Build-Strategie so, dass das gar nicht passieren kann.
- [ ] **Restart-Success-Check serverseitig (F6 aus Audit).** Aktuell: `_fire_and_forget` feuert `systemctl restart`, Response geht raus bevor Restart greift. Frontend pollt `/info` und verifiziert die neue Version — reicht für Welle 1. Langfristig: Server-seitiger Post-Start-Healthcheck, der „success"-Response erst liefert, wenn der neue Stand läuft.
- [ ] **`core/settings.py:30` Host-Default auf 127.0.0.1.** systemd-Unit überschreibt zwar via `--host 127.0.0.1`, aber der Default-Wert ist noch `0.0.0.0`. Kein aktiver Impact, Hygiene-Fix.
- [ ] **install.sh Sudo-Hinweis bei Direkt-Root-Aufruf.** Wenn Script ohne `sudo` als root gestartet wird (z. B. via `sudo su -`), ist `$SUDO_USER` evtl. leer → Fallback `pi`. Auf Bookworm-Installationen ohne `pi`-User: sudoers-Regel matcht nie, Restart-Feature silent broken. Abort-Pfad bereits vorhanden, aber unklar formuliert — Hint einbauen „Bitte `sudo bash install.sh` nutzen".
- [ ] **install.sh mktemp-Trap.** `trap 'rm -f "$SUDOERS_TMP"' EXIT` robuster als doppelte manuelle `rm -f`. Derzeit kein globaler Trap, risikofrei nachziehbar.
- [ ] **Update-Dialog Dismiss + Versions-Delta (UX-Follow-up aus Review).** Success-Auto-Dismiss 3 s ist knapp für Eltern, die während der 2-min-Installation anderes tun. Entweder Auto-Dismiss auf 30 s, oder explizites „Fertig"-Bestätigen. Zusätzlich: `{alt} → {neu}`-Versionsanzeige plus zugänglicher Link „Was ist neu?" mit Changelog. `no_changes`-Wording freundlicher („Alles erledigt — du bist auf vX" statt „Box ist bereits aktuell").

## Update-System-Härtung (Post-Beta, aus Security-Audit)

Die Welle 1 schließt die HIGH-Findings F1 (Rate-Limit), F2 (sudoers), F5 (Localhost-Binding), F7 (Cache-Control). Diese Findings bleiben offen:

- [ ] **F3 — Release-Signatur / Checksum-Verifikation.** `git pull` zieht aktuell unsigniert. Bei Kompromittierung des Upstream-Repos keine Gegenwehr. Minimal: signierte Git-Tags + `git verify-tag` vor Apply. Komplett: SHA-256-Manifest pro Release, im UI sichtbar. Sinnvoll parallel zur Umstellung auf GitHub-Releases-API (siehe „Prio 2.5 — Update-System ausbauen").
- [ ] **F4 — Atomic Update via Staging-Dir + Symlink-Swap.** Aktuell: In-place `git pull` hinterlässt bei Fehlschlag inkonsistenten Tree. Zielbild: Download + pip install in `releases/<version>/`, atomarer Symlink-Swap `current -> releases/<version>`, Boot-Failure-Rollback via systemd `OnFailure=`. Überschneidet sich mit Prio 2.5 und sollte dort gebündelt bearbeitet werden.
- [ ] **F8 — Git-Arg-Hardening.** `_git(*args)` nimmt Variadic; derzeit alle Aufrufe mit Literalen, keine Injection. Bei künftiger Erweiterung (Custom-Branch-Update o.ä.) Allowlist für Top-Level-Kommandos, Argumente explizit whitelisten.

## Pi-Image Folge-Items (aus Pi-Image-Welle 2026-04-23)

Die erste Image-Welle (`scripts/pi-gen-stage/stage-tonado/` + First-Boot-Logik + Wizard-Abschluss) ist bewusst auf „lokal baubar" beschnitten. Diese Items ziehen das Bild zum automatisierten, signierten Release-Image:

- [ ] **GitHub Action für Image-Build (`.github/workflows/pi-image.yml`).** x86-Runner + QEMU, Matrix arm64 + armhf, Trigger auf `v*`-Tags und `workflow_dispatch`. Caching für pi-gen-Stages und apt. Upload als Release-Asset via `gh release upload --clobber`. Struktur und YAML-Skeleton in [`docs/fuer-entwickler/pi-image-ci.md`](docs/fuer-entwickler/pi-image-ci.md).
- [ ] **QEMU-Smoke-Test in CI.** Nach dem Image-Build arm64-Variante in qemu-system-aarch64 booten, `curl -f http://localhost:8080/api/health` prüfen. Armhf in QEMU ist fragiler, nur arm64 testen. ~5 min pro Run.
- [ ] **Cosign keyless Signing + `actions/attest-build-provenance`.** Null-Aufwand-Signatur via GitHub OIDC, User kann mit `cosign verify-blob` prüfen. SHA256 bleibt zusätzlich. GPG-Schlüsselverwaltung vermeiden.
- [ ] **Eltern-Doku `docs/fuer-eltern/flashen.md`.** 3-Schritte-Anleitung mit Screenshots des Raspberry Pi Imager. Teil der ersten Release-Kandidat-Welle (`v0.4.0-beta`). Heute noch nicht geschrieben, weil der Image-Build-Pfad erst mit der CI-Welle stabil wird.
- [ ] **Release-Tag GPG-Signieren.** Aktuell ist der Tonado-Image-Build auf Commit-SHA-Pinning (`TONADO_EXPECTED_SHA`) angewiesen, weil Tags unsigniert sind. Sobald der Release-Prozess signierte Tags produziert, im pi-gen Chroot-Script `git verify-tag "${TONADO_TAG}"` aktivieren und den SHA-Pin optional machen.
- [ ] **Custom-Repository-Eintrag im Raspberry Pi Imager (post-Beta).** Schema V4-JSON (siehe Research-Report), Eltern wählen Tonado direkt im Imager-OS-Picker. Erst lohnend, wenn Antrag auf Upstream-Aufnahme in `raspberrypi/rpi-imager` läuft.
- [ ] **SSH-Hardening: Default Key-Only (Security F10).** Heute ist SSH an und akzeptiert sowohl Passwort (wenn vom Imager gesetzt) als auch Key. Nach Pre-Beta-Audit empfohlen: `PasswordAuthentication no` als Toggle im Setup-Wizard (nach Heim-WLAN-Connect). Bastler können es an lassen, Standard-Eltern sind Key-Only.
- [ ] **Reset-Taster Hardware-Weg.** Im Alltag: Heim-WLAN wechselt, Box hängt im Nirgendwo. Heutiger Fallback ist die WLAN-Rettung (Box spannt automatisch AP auf nach Timeout). Ergänzung: physischer Reset-Knopf (GPIO) zwingt in Setup-Modus — für den Fall, dass die Rettung nicht automatisch greift.
- [ ] **Demo-Content — leer lassen ist die Entscheidung, aber dokumentieren.** Erste Box zeigt keine Musik. Radio/Podcast-Katalog (Prio 3) füllt das in späterer Welle. Eltern-Doku soll den Erwartungshorizont setzen: „Lade deine Musik hoch oder stöber im Radio-Katalog."

## Pi-Image Härtung (aus AP-Konsolidierungs-Welle 2026-06-10)

Findings aus dem 4-Brillen-Review der Boot-Fix-Welle, bewusst auf Post-Beta verschoben (kein Release-Blocker; die Welle macht das Image boot- und setup-fähig):

- [ ] **Inaktivitäts-Timeout für den offenen Setup-AP.** Der Setup-AP ist offen (Henne-Ei: Eltern haben noch keine Creds) und systemd-owned ohne Zeitlimit — er bleibt offen, bis `.setup-complete` geschrieben ist. Bei nie abgeschlossenem Setup bleibt das Funk-Fenster dauerhaft offen; ein Nachbar in Reichweite könnte den Wizard bedienen (Heim-PSK-Bruteforce ist via `PROBE_FAIL_LOCKOUT=10` gedeckelt, das Hijack-Fenster aber nicht). Lösungsidee: Watchdog/`RuntimeMaxSec`, der nach N Stunden ohne Setup-Fortschritt rebootet. Security-Review-MEDIUM.
- [ ] **PSK-Übergabe an setup-ap.sh härten (argv → stdin).** `captive_portal.py` ruft `sudo -n setup-ap.sh start secured <ssid> <psk>` — das Recovery-AP-PSK ist kurz in `/proc/*/cmdline` + `auth.log` sichtbar (nur root/pi lokal). Akzeptabel für einen LAN-Recovery-AP, aber sauberer wäre PSK über stdin/Datei in `/run` statt argv. Security-Review-LOW.
- [ ] **WLAN-Regdomain-Persistenz auf Bookworm/NetworkManager verifizieren.** Der Setup-AP selbst kommt hoch (hostapd `country_code=DE` + rfkill-unblock in firstrun). Offen: ob der `wpa_supplicant.conf`-`country=`-Fallback in `03-tonado-config` auf einem NetworkManager-System überhaupt greift (NM konsumiert die Datei evtl. nicht) — dann wäre der STA-Modus (Heim-WLAN, 5-GHz-Kanäle) regdomain-eingeschränkt. Auf echter HW prüfen, ob `raspi-config nonint do_wifi_country DE` reboot-persistent wirkt; ggf. NM-natives Setzen ergänzen oder den toten Fallback ehrlich kommentieren. Network-Review-MEDIUM.

## Pi-Image Headless-UAT (v0.4.0-beta, 2026-06-11)

Erste echte Headless-UAT des geflashten arm64-Images auf Pi 3B+ (kein Monitor/Tastatur, [[feedback_test_as_shipped_artifact]]). Zwei Boot-Blocker gefunden **und gefixt** (Code committet, **noch nicht gepusht** — 3 Commits vor origin: `7bfdbea` Release-vor-Upload, `a6b6aca` Exec-Bit, `32e38b2` Deadlock). Danach drei nachgelagerte Wizard-Befunde (schichtweise aufgetaucht, [[feedback_blocker_maskiert_latente_bugs]]).

### Gefixt (Boot — auf echter HW validiert)

- [x] **Exec-Bit auf `system/*.sh`** (`a6b6aca`). Windows-Git committete die Runtime-Skripte `100644` → systemd `ExecStart` scheiterte `203/EXEC`, firstrun + Setup-AP starteten nie, Box headless-unerreichbar. Fix 3-schichtig (git +x, Stage-02-`chmod`, qemu-Smoke-Assert).
- [x] **systemd-Deadlock in `firstrun.sh`** (`32e38b2`). `Before=ssh.service`, rief aber synchron `systemctl start ssh.service` von innen → zirkulärer Deadlock, wedgte die ganze systemd-Job-Queue, `multi-user.target` nie erreicht. War hinter dem Exec-Bit-Bug maskiert. Fix: firstrun fasst `ssh*` nicht mehr per systemctl an; `Before=` (jetzt inkl. `ssh.socket`) regelt die Reihenfolge ohnehin.
- **Folge:** Tag `v0.4.0-beta` (@ `c05fef1`) enthält diese Fixes **nicht** → vor Release-Publish muss das Image neu gebaut werden (Beta-Tag neu schneiden oder `v0.4.1-beta`). Headless-Boot bis „Tonado-Setup"-AP + Captive-Portal-Auto-Open ist auf Pi 3B+ jetzt bewiesen.

### Offen (Wizard-Rework — Richtung: Offline-First-Fork)

PO-Entscheidung 2026-06-11: **Offline-First-Fork.** Der Wizard bietet künftig explizit zwei Wege — „Mit Heim-WLAN verbinden" (für Streams/Podcasts/Updates) vs. „Ohne Internet nutzen". Querverweis: bestehende Prio-2-Items „Offline-Modus" + „WLAN-Verwaltung in Einstellungen" gehen hierin auf ([[feedback_validate_backlog_use_case]]).

- [ ] **Offline-Modus als erstklassiger Wizard-Pfad.** Backend `/api/setup/complete` verlangt **kein** Heim-WLAN (nur Eltern-PIN) — Offline ist serverseitig schon möglich, es fehlt der Wizard-Weg. Offline-Pfad: Scan/Probe/QR überspringen, das gesicherte Recovery-WLAN „Tonado" (SSID+PSK aus dem Recovery-Schritt, existiert bereits) zum **dauerhaften** Netz machen. `/complete` muss im Offline-Fall den AP **an lassen** statt `portal.stop()` + Monitor anders/nicht armen (heute stoppt es den Portal-AP und armt den ConnectivityMonitor — beides setzt Heim-WLAN voraus). Lokale Dateien + RFID + Gyro laufen voll offline; Streams/Podcasts/Updates klar als „braucht Internet" kommunizieren.
- [ ] **WLAN-Scan im Setup liefert leere Liste (Single-Radio) → Boot-Scan-Cache + manuelle Eingabe.** Ursache: `_nmcli_scan` macht `nmcli device wifi list`, aber `setup-ap.sh` hat `wlan0` per `managed no` in den AP-Modus genommen → NM hat kein verwaltetes WLAN-Gerät → leere Liste. **Hardware-Fakt (verifiziert 2026-06-11):** Pi 3B+, 4 UND 5 nutzen denselben WLAN-Chip **CYW43455** — ein Funkmodul, ein Kanal. Pi 5 hat nur eine schnellere SDIO-Anbindung, kein zweites Radio. Concurrent AP+STA ist zwar möglich, aber zwingend single-channel; ein Scan muss off-channel → stört/killt den AP, und auf dem Pi 5 kann wiederholtes Concurrent `brcmfmac` in einen nur-per-Kaltstart-erholbaren Zustand fahren. „AP + freier Scan" läuft auf **keinem** Zielgerät zuverlässig — die Limitierung ist flottenweit, kein 3B+-Sonderfall. **Beschlossenes Design (PO + Tech-Lead, 2026-06-11):** (a) **Boot-Scan-Cache** — beim Boot im Fenster VOR dem AP (`imager-wifi-probe` läuft `Before=tonado-ap`, `wlan0` dort noch NM-verwaltet) einmal `nmcli dev wifi list` scannen und nach `/run/tonado/wifi-scan.json` cachen; der Wizard zeigt diese echte Liste ohne den AP zu stören. (b) **Manuelle SSID-Eingabe** als immer sichtbarer Fallback („Netz nicht dabei? Namen eingeben") — null Scan, 100% zuverlässig. (c) **Kein** Live-drop-scan-restore in v1 (man kann das Ergebnis nicht über das gerade abgeschaltete Radio zurückliefern; Handy-Reconnect zum offenen AP unzuverlässig, v.a. iOS). Falls später Refresh gewünscht: die vorhandene QR-/Dual-WLAN-Reconnect-Choreografie wiederverwenden.
- [ ] **Connect-Realität abbilden: „AP bleibt oben während Heim-WLAN-Probe" ist Single-Radio-unmöglich.** Die heutige Annahme (`probe_home_wifi` + `home-wifi-active`-Flag testen das Heim-WLAN, während der AP angeblich oben bleibt) widerspricht dem Ein-Radio-Chip — physisch nicht machbar. Richtiges Modell: beim Verbinden **AP runter → Heim-WLAN joinen → Handy findet die Box via QR im Heimnetz wieder** (die QR-/Dual-WLAN-Maschinerie existiert bereits, nur die „AP bleibt oben"-Prämisse muss raus). Im Wizard-Rework zusammen mit dem Offline-Fork klären.
- [ ] **Audio-Testton bleibt stumm — Erkennung ≠ Aktivierung.** `setup_audio()` schreibt nur `audio.device` in die Config, lädt **kein** `dtoverlay`. HifiBerry-DAC wird erkannt (EEPROM/I2C), aber das Overlay ist im Image nicht gebacken (opt-in at runtime) → ALSA hat kein funktionierendes Ausgabegerät → `aplay` ins Leere. Fix: Audio-Step muss das passende `dtoverlay` in `config.txt` schreiben (+ Onboard `dtparam=audio=on` deaktivieren wenn DAC) und einen Reboot anstoßen, damit die Auswahl real wirkt.
- [ ] **Wizard-Endschritt: Endlossuche ohne Heim-WLAN-Wechsel.** „Jetzt einrichten" triggert stur Probe + QR + Box-Suche im neuen Netz; ohne gewähltes Heim-WLAN (s.o. leerer Scan) sucht es endlos. Mit dem Offline-Fork entfällt das für den Offline-Pfad; für den Heim-WLAN-Pfad braucht es einen harten Timeout + verständlichen Fehlerausgang (existiert teils via `confirm-complete`-Token/Lockout, aber die UI-Sackgasse muss weg).
