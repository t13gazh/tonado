...
- [ ] **QEMU-Smoke-Test (`scripts/ci/qemu-smoke.sh`) ist dauerhaft rot — Check-Bugs, nicht Image-Bugs (MEDIUM).** Der Job ist `continue-on-error: true` und war bei JEDEM Build rot (auch dem ersten grünen), liefert also kein echtes Gate. Verifizierte Check-Fehler: (a) **Unit-Check** nutzt `[ -e $R/etc/systemd/system/<unit>.service ]` — die Stage legt aber ABSOLUTE Symlinks (`ln -sf /opt/tonado/system/<unit> …`), denen `-e` vom gemounteten Rootfs aus auf den CI-Host folgt (wo nichts liegt) → False-FAIL. Fix: `-L` statt `-e` (oder Symlink-Ziel relativ zum Mount auflösen). (b) **cmdline.txt-Checks** (single-line, ipv6.disable, init=-resize) laufen in der Offline-Phase, BEVOR die Boot-Partition `p1` gemountet ist → lesen die falsche/leere Datei. Fix: cmdline.txt von `${LOOP}p1` lesen. (c) `build-essential purged` / `distro dnsmasq not enabled` gegenprüfen. Bis dahin: Image-Korrektheit per echter HW verifizieren, nicht per Smoke-Test.
