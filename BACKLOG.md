...
## Update-Mechanismus (Post-Beta, Known-Issues aus 0.3.0-beta-Live-Test)

Beim ersten Live-Test des `/api/system/update/apply`-Endpoints am 2026-04-22 (Upgrade 0.2.1-alpha → 0.3.0-beta auf Pi 3B+) sind drei Schwächen aufgefallen, die vor 0.3.1-beta geprüft und sauber abgedeckt werden sollten:

- [ ] **Apply schlägt in bestimmten Working-Tree-Zuständen leise fehl.** Im Live-Test gab der Endpoint `200 OK` zurück, das Backend hat die Dateien teilweise in den Ziel-Zustand überführt, aber der `HEAD`-Pointer wurde nicht bewegt — `git log` zeigte weiter den alten Commit, `pyproject.toml` aber schon die neue Version. Im Journal kein Exception-Trace, kein „Update applied"-Log. Root-Cause-Analyse offen. Startpunkte: `core/services/system_service.py::_apply_update_unlocked` Zeilen 263-298, Verhalten von `git pull --ff-only` wenn der Working-Tree bereits modifiziert ist, Interaktion mit `_fire_and_forget("sudo", "systemctl", "restart", ...)` — eventuell killt der Service-Restart den eigenen Pull mittendrin, bevor Index + HEAD atomar gesetzt sind.
- [ ] **Update-UI gibt keinen sichtbaren Progress-Status.** Nach dem Klick auf „Jetzt installieren" ist im UI kein Spinner, keine Status-Zeile, kein „läuft"-Feedback sichtbar. Der Nutzer weiß nicht, ob das System arbeitet, ob es hängt, ob es fertig ist. Der `message`-State wird zwar gesetzt (`updating=true`, `message = update_installing`), aber vermutlich vom Layout verdeckt oder zu dezent gestylt. Inline neben dem Button einen klaren Status-Block platzieren: „Wird installiert …" → „Fertig, Neustart …" → „Auf vX.Y.Z aktualisiert". Mit minimaler Animation.
- [ ] **`Cache-Control: no-store` auf `/api/system/update/check`.** Beobachtet am Live-Test: Browser hat die Update-Check-Response nach einem CHANGELOG-Fix weiter mit dem alten Body gecached, bis ein hartes „Clear site data" gemacht wurde. Der Response-Header sollte explizit `no-store` setzen (bzw. `Cache-Control: private, no-cache, no-store, must-revalidate`), damit „Nach Updates suchen" immer frisch geladen wird. Ähnliche Header-Discipline auch für andere volatile Status-Endpoints prüfen.
- [ ] **Frontend-Asset-Hash-Konsistenz bei Rebuilds.** Im Live-Test hat ein `git reset --hard` auf einen frühen Commit mit danach durchgeführtem `git clean -fdx` einen Zustand produziert, in dem `index.html` auf einen App-Bundle-Hash verwies, der im `_app/immutable/entry/`-Ordner nicht mehr existierte → weißes Frontend, „Failed to load module script". Im normalen Git-Workflow (kein reset mischen) ist das nicht reproduzierbar, aber es ist fragil. Entweder den Build-Prozess robuster machen (alle generierten Files müssen in genau einem Rebuild-Commit pro Source-Wave stecken) oder einen `prestart`-Hook im Service, der beim Start prüft, ob `index.html` alle referenzierten Assets findet — und bei Diskrepanz einen sauberen `npm run build` triggert.
