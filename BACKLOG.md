...
- [ ] **WLAN-Scan im Setup liefert leere Liste (Single-Radio).** `_nmcli_scan` macht `nmcli device wifi list`, aber `setup-ap.sh` hat `wlan0` per `managed no` in den AP-Modus genommen → NM hat kein verwaltetes WLAN-Gerät → leere Liste. Auf dem Pi-Funkchip (brcmfmac) ist „gleichzeitig AP **und** scannen" nicht möglich. **Empfehlung (Tech-Lead): manuelle SSID-Eingabe** im Heim-WLAN-Pfad statt Scan (zuverlässig auf einem Radio); die Box gibt dann beim Verbinden den AP frei, Handy kommt via QR/Dual-WLAN-Flow zurück. Alternativ: AP kurz droppen → scannen → AP wieder hoch (stört die Portal-Verbindung).
