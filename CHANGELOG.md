...
- **Figur löschen nur noch im aufgeklappten Detail, nicht auf der Kachel.** Das kleine Lösch-Icon direkt auf der Figuren-Kachel ist weg — versehentliches Löschen beim Streifen über das Grid war zu leicht. Stattdessen gibt es einen klar beschrifteten „Figur löschen"-Bereich im Edit-Fenster, unterhalb der normalen Aktionen mit einer Trennlinie abgesetzt. Die Sicherheits-Rückfrage bleibt wie gehabt.
- **Bewegungssteuerungs-Schalter zeigt „An / Aus" statt „Aktiviert / Deaktiviert".** Konsistenter Wortlaut mit dem Rest der Einstellungen.
- **WLAN-Rettungs-Intervall sieht aus wie die anderen Auswahl-Leisten.** Die ausgewählte Zeit (5 Min, 15 Min usw.) hatte einen Ring mit Abstand außen, der in engen Bereichen optisch abgeschnitten wurde. Jetzt einheitlich zu den anderen Segmentauswahlen auf der Seite.
- **Fehler-Meldungen in Einstellungen und Setup-Wizard sehen überall gleich aus.** Einzelne rote Zeilen wurden durch die bereits etablierte Inline-Error-Box mit Icon ersetzt — kein Stilflickwerk mehr zwischen den Schritten.
- **„Box herunterfahren" ist vom Rest der Systemaktionen visuell abgetrennt.** Eine dünne Linie markiert den destruktiven Bereich, damit der Shutdown-Knopf nicht versehentlich mit Neustart verwechselt wird.
- **Browser-Audio-Schalter bleibt mit dem gehörten Zustand synchron.** Nach einem Stream-Wechsel blieb der Schalter hängen und wirkte deaktiviert, obwohl das Audio ein paar Sekunden später von selbst zurückkam — der Button folgt jetzt dem tatsächlichen Abspielen statt einem veralteten Retry-Zustand. Parallele Retry-Versuche bei schnellen Wechseln stehen sich außerdem nicht mehr gegenseitig im Weg.
- **„Einrichtung neu starten" erklärt den Weg zur Experten-PIN jetzt ehrlich.** Der alte Hinweis sagte „melde dich an" — aber wenn noch gar keine PIN gesetzt ist, kann man sich ja nicht anmelden. Jetzt heißt es sinngemäß: „Erst PIN unter Sicherheit anlegen, dann als Experte anmelden."
- **Fokus-Umrandung wird in keinem Eingabefeld mehr abgeschnitten.** Ein scrollender Wizard-Container hat den äußeren Focus-Ring links/rechts geclippt. Eingabefelder, Textareas und Dropdowns übernehmen jetzt ihr eigenes Fokus-Feedback (Border-Farbe, in manchen Fällen ein zusätzlicher innenliegender Ring); die globale Keyboard-Fokus-Markierung für alle anderen Elemente bleibt unverändert.
