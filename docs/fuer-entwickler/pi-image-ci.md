...
Status: **Implementiert** in [`.github/workflows/pi-image.yml`](../../.github/workflows/pi-image.yml) (2026-05-29), noch nicht durch einen echten CI-Run verifiziert — der erste Lauf erfolgt mit dem `v0.4.0-beta`-Tag oder per `workflow_dispatch`.

> **Abweichung zu diesem RFC:** Abschnitt 3 unten beschreibt einen **verworfenen** Tarball-Ansatz mit npm-Build im Runner. Die reale Stage ([`scripts/pi-gen-stage/stage-tonado/02-tonado-code`](../../scripts/pi-gen-stage/stage-tonado/02-tonado-code/00-run-chroot.sh)) holt den Code per `git clone --branch <tag>` **im Chroot** und installiert `.[pi]` dort; `web/build/` ist im Repo committed, der Workflow baut kein Frontend. Der Workflow übergibt nur `TONADO_VERSION` + `TONADO_EXPECTED_SHA` via pi-gen-`config`. Weitere bewusste Abweichungen: kein `pi-gen/work`-Cache (Korrektheit vor Geschwindigkeit beim Bring-up); QEMU-Smoke-Test ist `continue-on-error` und primär eine Offline-Rootfs-Inspektion (siehe [`scripts/ci/qemu-smoke.sh`](../../scripts/ci/qemu-smoke.sh)); armhf-`pi-gen`-Ref ist noch `master` statt SHA-gepinnt (TODO vor erstem armhf-Release).
